...
class BaseContinuousAlgo:
    ...
    def collect_experiences(self):
        ...
        for i in range(self.num_frames_per_proc):
            ...
            with torch.no_grad():
                if self.acmodel.recurrent:
                    probas, dist, value, memory = self.acmodel(
                        preprocessed_obs, self.memory * self.mask.unsqueeze(1)
                    )
                else:
                    probas, dist, value = self.acmodel(preprocessed_obs)
            action = probas
        ...
        with torch.no_grad():
            if self.acmodel.recurrent:
                ...
                _, _, next_value = self.acmodel(preprocessed_obs)
        ...
        exps.action = (
            ...
            .transpose(0, 1)
            .reshape(self.num_frames_per_proc * self.num_procs, -1)
        )
...
class ContinuousPPOAlgo(BaseContinuousAlgo):
        ...
        for _ in range(self.epochs):
            ...
            for inds in self._get_batches_starting_indexes():
                ...
                for i in range(self.recurrence):
                    ...
                    if self.acmodel.recurrent:
                        _, dist, value, memory = self.acmodel(sb.obs, memory * sb.mask)
                    else:
                        _, dist, value = self.acmodel(sb.obs)
            ...
            if self.batch_num % self.action_std_decay_freq == 0:
                self.acmodel.update_action_std()
