...
class AdversaryACModel(nn.Module, torch_ac.RecurrentACModel):
    def __init__(self, env):
        ...
        # Define actor's model
        self.actor = nn.Sequential(
            nn.Linear(self.feature_size, 64), nn.Tanh(), nn.Linear(64, env.state_size)
        )

        # Define critic's model
        self.critic = nn.Sequential(
            nn.Linear(self.feature_size, 64), nn.Tanh(), nn.Linear(64, 1)
        )
    ...
    def forward(self, obs, memory):
        ...
        x = self.actor(embedding)
        probas = x.exp()
        probas[obs.transition_probas[:, :-1] == 0] = 0
        probas /= probas.sum()

        x = self.critic(embedding)
        value = x.squeeze(1)

        return probas, value, memory
